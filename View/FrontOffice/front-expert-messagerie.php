...
<head>
    ...
    <style>
        ...
        .msg-actions button:hover { color: var(--secondary); border-color: var(--secondary); }
...
<div class="dashboard-container">
    <aside class="sidebar">
        <div class="sidebar-header">
            <a href="index.php" class="logo" style="text-decoration:none;"><i class="fa-solid fa-chart-pie text-secondary"></i> Digit Advisory</a>
        </div>
        ...
        <div class="user-profile-widget">
            ...
            <div>
                ...
                <span style="font-size:.8rem;color:var(--gray);">Consultant Expert</span>
            </div>
            ...
            <div class="chat-wrap">
                <div class="chat-sidebar">
                    <div class="chat-sidebar-head">
                        <h3>Conversations</h3>
                        <button class="btn" style="background:var(--secondary);color:white;padding:.35rem .75rem;font-size:.8rem;border-radius:8px;border:none;cursor:pointer;" onclick="openNewConvModal()">
                ...
                <div class="chat-main" id="chatMain">
                    <div class="chat-empty" id="chatEmpty">
                        ...
                        <button class="btn" style="background:var(--secondary);color:white;border:none;padding:.6rem 1.25rem;border-radius:8px;cursor:pointer;" onclick="openNewConvModal()">
...
const API   = '<?= $apiUrl ?>';
const MY_ID = <?= $myId ?>;
let currentConvId = null, pollingTimer = null, allConvs = [], allUsers = [];
...
function loadConvs() {
    fetch(API + '?action=get_conversations')
        .then(r => r.json())
        .then(data => { allConvs = data; renderConvs(data); });
}
...
function openConv(id, name, initials, role, otherId) {
    ...
    document.getElementById('chatHeader').innerHTML = `
        <div style="width:38px;height:38px;border-radius:50%;background:var(--primary);color:white;display:flex;align-items:center;justify-content:center;font-weight:700;font-size:.85rem;">${escHtml(initials)}</div>
    ...
    if (m.is_deleted) {
        ...
        if (isImg) {
            ...
            body = `<a class="msg-file" href="../../${m.file_path}" target="_blank" download="${escHtml(m.file_name)}">
                <i class="${fileIcon(m.file_name)}"></i><span>${escHtml(m.file_name)} ${m.file_size?'('+formatSize(m.file_size)+')':''}</span></a>`;
        }
    }
    const actions = (isMine && !m.is_deleted) ? `
        <div class="msg-actions">
            ${m.type==='text'?`<button onclick="startEdit(${m.id_message})"><i class="fa-solid fa-pen"></i></button>`:''}
            <button class="del" onclick="deleteMsg(${m.id_message})"><i class="fa-solid fa-trash"></i></button>
        </div>` : '';
...
function sendText() {
    ...
    fd.append('action','send_message'); fd.append('id_conversation',currentConvId);
    fd.append('type','text'); fd.append('content',v);
...
function sendFile(file) {
    ...
    const fd=new FormData();
    fd.append('action','send_message'); fd.append('id_conversation',currentConvId);
    fd.append('type','file'); fd.append('file',file);
    document.getElementById('fileInput').value='';
    fetch(API,{method:'POST',body:fd}).then(r=>r.json()).then(d=>{if(d.success){loadMessages(currentConvId);loadConvs();}else{alert(d.error||'Erreur envoi fichier');}});
}
...
function startRecording() {
    navigator.mediaDevices.getUserMedia({audio:true}).then(stream=>{
        ...
        mediaRecorder.onstop=()=>{
            ...
            sendAudio(new File([blob],'voice_'+Date.now()+'.webm',{type:'audio/webm'}));
...
function sendAudio(file) {
    const fd=new FormData();
    fd.append('action','send_message'); fd.append('id_conversation',currentConvId);
    fd.append('type','audio'); fd.append('file',file);
...
function confirmEdit(id,v,orig) {
    ...
    const fd=new FormData();
    fd.append('action','edit_message'); fd.append('id_message',id); fd.append('content',v);
    fetch(API,{method:'POST',body:fd}).then(r=>r.json()).then(()=>loadMessages(currentConvId));
}

function cancelEdit(id,inp,orig) {
    const span=document.createElement('span');
    span.className='msg-content'; span.dataset.id=id; span.textContent=orig;
...
function deleteMsg(id) {
    ...
    const fd=new FormData();
    fd.append('action','delete_message'); fd.append('id_message',id);
    fetch(API,{method:'POST',body:fd}).then(r=>r.json()).then(()=>loadMessages(currentConvId));
}
...
function confirmDeleteConv(id) {
    ...
    const fd=new FormData();
    fd.append('action','delete_conversation'); fd.append('id_conversation',id);
...
function renderUserList(users) {
    ...
    if(!users.length){ul.innerHTML='<div style="text-align:center;color:var(--gray);padding:1rem;">Aucune entreprise disponible</div>';return;}
    ul.innerHTML=users.map(u=>`
        <div class="user-list-item" onclick="startNewConv(${u.id_user})">
            <div class="ua" style="background:var(--primary);">${escHtml(u.initials)}</div>
            <div>
                <strong style="font-size:.9rem;">${escHtml(u.display_name)}</strong>
                <p style="font-size:.78rem;color:var(--gray);margin:0;">${escHtml(u.domaine||'Entreprise')}</p>
            </div>
        </div>`).join('');
}
...
function startNewConv(otherId) {
    closeNewConvModal();
    const fd=new FormData();
    fd.append('action','create_conversation'); fd.append('id_other_user',otherId);
...
function fileIcon(name) {
    ...
    if(['mp3','ogg','wav','webm'].includes(ext)) return 'fa-solid fa-file-audio';
