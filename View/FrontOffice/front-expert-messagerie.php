...
$me     = $_SESSION['user'];
$myId   = (int)$me['id_user'];
$myName = trim(($me['prenom']??'').' '.($me['nom']??'')) ?: $me['email'];
$myInit = strtoupper(mb_substr($me['prenom']??'E',0,1)).strtoupper(mb_substr($me['nom']??'',0,1));
...
<head>
    ...
    <style>
        ...
        .menu-item:hover,.menu-item.active{background:rgba(14,165,233,.05);color:var(--secondary)}
        ...
        .user-avatar{width:40px;height:40px;border-radius:50%;background:var(--secondary);color:white;display:flex;justify-content:center;align-items:center;font-weight:600;font-size:.85rem}
        ...
        .conv-item.active{background:#e0f2fe}
        ...
        .unread-badge{background:var(--secondary);color:white;border-radius:999px;font-size:.65rem;font-weight:700;padding:.1rem .45rem;min-width:18px;text-align:center}
        ...
        .msg-btn:hover{color:var(--secondary);border-color:var(--secondary)}
        ...
        /* FIX: textarea min-height garantie */
        ...
        .input-btn.attach:hover{background:#e2e8f0;color:var(--secondary)}
        ...
        .input-btn.send{background:var(--secondary);color:white;width:38px;height:38px}
        .input-btn.send:hover{background:#0284c7}
        ...
        .modal-search:focus{border-color:var(--secondary)}
        ...
        .user-list-item:hover{background:#f0f9ff}
...
<div class="dashboard-container">
    <aside class="sidebar">
        ...
        <div class="user-profile-widget">
            ...
            <div><h4 style="font-size:.95rem;margin-bottom:.2rem;"><?=htmlspecialchars($myName)?></h4><span style="font-size:.8rem;color:var(--gray);">Consultant Expert</span></div>
            ...
            <div class="chat-wrap">
                <div class="chat-sidebar">
                    <div class="chat-sidebar-head">
                        <h3>Conversations</h3>
                        <button style="background:var(--secondary);color:white;border:none;padding:.35rem .75rem;font-size:.8rem;border-radius:8px;cursor:pointer;" onclick="openNewConvModal()"><i class="fa-solid fa-plus"></i></button>
                    </div>
                ...
                <div class="chat-main">
                    <div class="chat-empty" id="chatEmpty">
                        ...
                        <button style="background:var(--secondary);color:white;border:none;padding:.6rem 1.25rem;border-radius:8px;cursor:pointer;" onclick="openNewConvModal()"><i class="fa-solid fa-plus"></i> Nouvelle conversation</button>
                    </div>
                    <div id="chatZone" style="display:none;flex:1;flex-direction:column;min-width:0;overflow:hidden;">
                        ...
                        <div class="chat-input-area">
                            ...
                            <button class="input-btn send" onclick="sendText()"><i class="fa-solid fa-paper-plane"></i></button>
                        </div>
                    </div>
                </div>
            </div>
...
<div class="modal-overlay" id="newConvModal">
    <div class="modal-box">
        <h3><i class="fa-solid fa-plus-circle" style="color:var(--secondary);"></i> Contacter une entreprise</h3>
        <input type="text" class="modal-search" id="userSearch" placeholder="Rechercher une entreprise..." oninput="filterUsers(this.value)">
...
const API   = '<?=$apiUrl?>';
const MY_ID = <?=$myId?>;
let currentConvId=null, pollingTimer=null;
...
function renderConvs(convs) {
    ...
    if(!convs.length){list.innerHTML='<div class="no-convs"><i class="fa-solid fa-comment-slash" style="font-size:2rem;opacity:.3;"></i><br>Aucune conversation</div>';return;}
    list.innerHTML=convs.map(c=>{
        ...
        return `<div class="conv-item${active}" data-id="${c.id_conversation}"
            ...
            <div class="conv-avatar" style="background:var(--primary);">${esc(c.other_initials)}</div>
...
function filterConvs(q){q=q.toLowerCase();renderConvs(allConvs.filter(c=>c.other_name.toLowerCase().includes(q)||(c.last_preview||'').toLowerCase().includes(q)));}
...
function openConv(id,name,initials,role,otherId) {
    ...
    document.getElementById('chatHeader').innerHTML=`
        ...
        <button onclick="confirmDeleteConv(${id})" style="background:none;border:none;color:var(--gray);cursor:pointer;font-size:1rem;margin-left:auto;"><i class="fa-solid fa-trash"></i></button>`;
    loadMessages(id);
    startPolling(id);
}
...
function buildMsg(m) {
    ...
    else if(m.type==='text') body=`<span class="msg-content" data-id="${m.id_message}">${esc(m.content)}</span>`;
    else if(m.type==='audio') body=`<audio class="msg-audio" controls preload="metadata" src="../../${m.file_path}"></audio>`;
    else if(m.type==='file'){
...
function updateStatus(online,typing,name) {
    const dot=document.getElementById('onlineDot'), lbl=document.getElementById('onlineLabel');
    if(dot) dot.className='online-dot '+(online?'on':'off');
    if(lbl) lbl.textContent=online?'En ligne':'Hors ligne';
    const ti=document.getElementById('typingIndicator'), tn=document.getElementById('typingName');
...
function sendText() {
    ...
    if(!v||!currentConvId) return;
    const fd=new FormData();
    fd.append('action','send_message');fd.append('id_conversation',currentConvId);fd.append('type','text');fd.append('content',v);
    inp.value=''; inp.style.height='44px'; // FIX reset
    clearTimeout(typingDebounce); sendTyping(0);
...
function onTypingInput(el) {
    ...
    el.style.height=Math.min(el.scrollHeight,120)+'px'; // FIX auto-resize
...
function sendTyping(convId){const fd=new FormData();fd.append('action','update_typing');fd.append('id_conversation',convId);fetch(API,{method:'POST',body:fd}).catch(()=>{});}
...
function sendFile(file) {
    ...
    fetch(API,{method:'POST',body:fd}).then(r=>r.json()).then(d=>{if(d.success){loadMessages(currentConvId);loadConvs();}else alert(d.error||'Erreur');});
}
...
function startRecording() {
    ...
    navigator.mediaDevices.getUserMedia({audio:true}).then(stream=>{
        ...
        mediaRecorder.start(50); // FIX timeslice
...
function confirmDeleteConv(id){if(!confirm('Supprimer cette conversation ?'))return;const fd=new FormData();fd.append('action','delete_conversation');fd.append('id_conversation',id);fetch(API,{method:'POST',body:fd}).then(r=>r.json()).then(()=>{currentConvId=null;stopPolling();renderHash='';document.querySelector('.chat-wrap').classList.remove('in-chat');document.getElementById('chatZone').style.display='none';document.getElementById('chatEmpty').style.display='';allConvs=allConvs.filter(c=>c.id_conversation!=id);renderConvs(allConvs);loadConvs();});}
...
function openNewConvModal(){document.getElementById('newConvModal').classList.add('open');document.getElementById('userSearch').value='';document.getElementById('userList').innerHTML='<div style="text-align:center;color:var(--gray);padding:1rem;"><i class="fa-solid fa-spinner fa-spin"></i></div>';fetch(API+'?action=get_users').then(r=>r.json()).then(users=>{allUsers=users;renderUserList(users);});}
function closeNewConvModal(){document.getElementById('newConvModal').classList.remove('open');}
function renderUserList(users){const ul=document.getElementById('userList');if(!users.length){ul.innerHTML='<div style="text-align:center;color:var(--gray);padding:1rem;">Aucune entreprise disponible</div>';return;}ul.innerHTML=users.map(u=>`<div class="user-list-item" onclick="startNewConv(${u.id_user})"><div class="ua" style="background:var(--primary);">${esc(u.initials)}</div><div><strong style="font-size:.9rem;">${esc(u.display_name)}</strong><p style="font-size:.78rem;color:var(--gray);margin:0;">${esc(u.domaine||'Entreprise')}</p></div></div>`).join('');}
...
function startNewConv(otherId){closeNewConvModal();const fd=new FormData();fd.append('action','create_conversation');fd.append('id_other_user',otherId);fetch(API,{method:'POST',body:fd}).then(r=>r.json()).then(d=>{if(d.success)setTimeout(()=>{fetch(API+'?action=get_conversations').then(r=>r.json()).then(convs=>{allConvs=convs;renderConvs(convs);const conv=convs.find(c=>c.id_conversation==d.id_conversation);if(conv)openConv(conv.id_conversation,conv.other_name,conv.other_initials,conv.other_role,conv.other_id);});},200);});}
...
function fmtTime(dt){if(!dt)return '';const d=new Date(dt.replace(' ','T')),now=new Date(),diff=Math.floor((now-d)/86400000);if(diff===0)return d.toLocaleTimeString('fr-FR',{hour:'2-digit',minute:'2-digit'});if(diff===1)return 'Hier';if(diff<7)return d.toLocaleDateString('fr-FR',{weekday:'short'});return d.toLocaleDateString('fr-FR',{day:'2-digit',month:'2-digit'});}
